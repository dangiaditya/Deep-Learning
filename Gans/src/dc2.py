...
class DCGAN(object):
    ...
    def __init__(self, batch_size=128, num_steps=100000, z_dim=100, gf_dim=64, df_dim=64,
                 input_channel=1):
    ...
    def create_placeholders(self):
        ...
        self.z_ = tf.placeholder(tf.float32, [16, self.z_dim], name="z_")
    ...
    def build(self):
        ...
        with self.graph.as_default():
            ...
            self.g_ = self.generator(self.z_, True)
            ...
            print("disc len", len(self.d_var_list))
            print("gen len", len(self.g_var_list))
            print("total", len(t_vars))
            for i in self.d_var_list:
                print("disc", i)
            for i in self.g_var_list:
                print("gen", i)
            for i in t_vars:
                print("lll", i)
            self.optimizer()
    ...
    def save_sample(gen_images, iteration=None):
        ...
        # gen_images = np.array(gen_images).reshape((16, 28, 28, 1))
    ...
    def optimizer(self):
        ...
        self.g_opt = tf.train.AdamOptimizer(learning_rate=0.0003, beta1=0.5).minimize(self.g_loss, var_list=self.g_var_list)
        self.d_opt = tf.train.AdamOptimizer(learning_rate=0.0003, beta1=0.5).minimize(self.d_loss, var_list=self.g_var_list)
    ...
    def generator(self, z, reuse=False):
        ...
        if batch_size is None:
            ...
            batch_size = 16
        print("asdfffffffffffff", batch_size)
        with tf.variable_scope("generator", reuse=reuse):
            ...
            shape_h2 = [batch_size, s_h8, s_w8, self.gf_dim*4]
            # print(shape_h2)
            h2 = self.ops.deconv(h1, shape_h2, name="gen_deconv1")
            h2 = tf.nn.relu(self.g_bn2(h2))

            shape_h3 = [batch_size, s_h4, s_w4, self.gf_dim * 2]
            h3 = self.ops.deconv(h2, shape_h3, name="gen_deconv2")
            h3 = tf.nn.relu(self.g_bn3(h3))

            shape_h4 = [batch_size, s_h2, s_w2, self.gf_dim]
            h4 = self.ops.deconv(h3, shape_h4, name="gen_deconv3")
            h4 = tf.nn.relu(self.g_bn4(h4))

            shape_h5 = [batch_size, s_h, s_w, self.in_channel]
...
#                print(z.shape)
                ...
                if i % 10 == 0:
                    print("\ngen_loss {}\ndisc_loss {}".format(d_loss, d_loss))
                    gen_images = self.get_noise(16)
                    gen_images = sess.run([self.g_], feed_dict={self.z_: gen_images})[0]
